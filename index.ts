document.addEventListener("DOMContentLoaded", () => {
  ...
  const screenElement = document.querySelector(
    "[data-screen]"
  ) as HTMLDivElement & { innerText: string };
  ...
  function handlingZeroFollowedByAdecimal(button: string): void {
    if (
      Number(button) === ZERO &&
      screenElement.innerText.startsWith(ZERO_DOT)
    ) {
  ...
  function userClicksOnEqualButton(button: string) : void {
    if (button === "=") {
      try {
        ...
        // unknown is safer than any because TypeScript forces you to narrow the type before using it (e.g., with instanceof checks).This prevents accidental assumptions about the error structure.
  ...
  function togglesClearHistoryButton(element: HTMLElement) : void {
